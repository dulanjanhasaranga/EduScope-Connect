...
public class DataSeeder implements CommandLineRunner {
        ...
        private void seedTags() {
                Map<String, List<String>> categories = Map.of(
                                "Programming & Tech", Arrays.asList(
                                                ...
                                                "ruby", "swift",
                                                "kotlin", "go", "rust", "dart", "scala", "perl", "haskell", "lua", "r",
                                                "matlab", "assembly",
                                                "shell", "powershell",
                                                "react", "angular", "vue", "svelte", "next.js", "nuxt.js", "gatsby",
                                                "html", "css", "sass",
                                                "less", "tailwind", "bootstrap", "node.js", "express", "django",
                                                "flask", "spring-boot",
                                                ...
                                                "jenkins",
                                                "github-actions", "gitlab-ci", "linux", "unix", "bash", "nginx",
                                                "apache",
                                                ...
                                                "distributed-systems", "software-engineering", "design-patterns",
                                                "agile", "scrum", "git",
                                                "github", "bitbucket"),
                                "Engineering & Science", Arrays.asList(
                                                "thermodynamics", "fluid-mechanics", "circuit-analysis", "calculus",
                                                "linear-algebra",
                                                "physics", "chemistry", "biology", "genetics",
                                                "statistics", "discrete-math", "differential-equations",
                                                "materials-science", "robotics",
                                                "aerospace", "civil-engineering", "mechanical-engineering",
                                                "astronomy", "environmental-science", "quantum-mechanics",
                                                "organic-chemistry", "biochemistry"),
                                "Medicine & Health", Arrays.asList(
                                                "anatomy", "pharmacology", "physiology", "pathology", "clinical-skills",
                                                "public-health",
                                                "nursing", "dentistry",
                                                "neuroscience", "immunology", "microbiology", "epidemiology",
                                                "pediatrics", "surgery",
                                                "psychiatry", "cardiology"),
                                "Business & Economics", Arrays.asList(
                                                "microeconomics", "macroeconomics", "accounting", "marketing",
                                                "management", "finance",
                                                "entrepreneurship",
                                                "corporate-finance", "investment", "business-analytics", "supply-chain",
                                                "human-resources",
                                                "strategy"),
                                "Arts & Humanities", Arrays.asList(
                                                "history", "literature", "philosophy", "sociology", "psychology",
                                                "art-history", "linguistics",
                                                "political-science",
                                                "anthropology", "ethics", "theology", "music-theory",
        ...
        private void seedSystemSettings() {
                if (systemSettingRepository.count() == 0) {
                        ...
                        List<SystemSetting> settings = Arrays.asList(
                                        SystemSetting.builder().key("MAINTENANCE_MODE").value("false")
                                                        .description("Enable to put the site into maintenance mode.")
                                                        .build(),
                                        SystemSetting.builder().key("ALLOW_PUBLIC_REGISTRATION").value("true")
                                                        .description("Allow new users to register an account.").build(),
                                        SystemSetting.builder().key("MAX_UPLOAD_SIZE_MB").value("50")
                                                        .description("Maximum allowed file upload size in MB.").build(),
                                        SystemSetting.builder().key("SUPPORT_EMAIL").value("support@eduscopeglobal.com")
        ...
        private void seedAuditLogs() {
                if (auditLogRepository.count() == 0) {
                        ...
                        List<AuditLog> logs = Arrays.asList(
                                        AuditLog.builder().action("UPDATED_SETTING").entityName("SystemSetting")
                                                        .entityId(1L).performedBy(admin)
                                                        .details("Changed MAINTENANCE_MODE from true to false")
                                                        .timestamp(LocalDateTime.now().minusDays(0)).build(),
                                        AuditLog.builder().action("DELETED_QUESTION").entityName("Question")
                                                        .entityId(99L).performedBy(admin)
                                                        .details("Deleted spam question 'buy cheap watches'")
                                                        .timestamp(LocalDateTime.now().minusHours(12)).build(),
                                        AuditLog.builder().action("PROMOTED_USER").entityName("User").entityId(3L)
                                                        .performedBy(admin)
                                                        .details("Promoted user erodriguez@research.edu to LEADER role")
                                                        .timestamp(LocalDateTime.now().minusHours(5)).build(),
                                        AuditLog.builder().action("CREATED_ECOSYSTEM_APP")
                                                        .entityName("EcosystemProduct").entityId(5L).performedBy(admin)
                                                        .details("Added RxCalculations product")
                                                        .timestamp(LocalDateTime.now().minusMinutes(30)).build());
        ...
        private void seedHistoricalAnswersForLeaders() {
                ...
                if (answerRepository.findByAuthor(admin).size() > 10)
                        return;
        ...
        private void seedAssessments() {
                ...
                a2.setQuestions(Arrays.asList(
                                com.educonnect.model.AssessmentQuestion.builder()
                                                ...
                                                .options(Arrays.asList("Decreased FEV1/FVC", "Increased TLC",
                                                                "Decreased TLC"))
        ...
        private void seedNewStudentsAndActivity() {
                ...
                String[] firstNames = { "Shenal", "Yenuli", "Nethmi", "Kaveesha", "Malshan", "Dinuka", "Sachini",
                                "Tharindu", "Kasun", "Piumi", "Dulmini", "Chathura", "Sandun", "Nishantha", "Hasini",
                                "Isuru", "Supun", "Chamara", "Nuwan", "Rumesh" };
                String[] lastNames = { "Kavindu", "Himasha", "Nisansala", "Dilshan", "Fernando", "Perera", "Silva",
                                "Kumara", "Jayasooriya", "Rajapaksha", "Gunathilaka", "Bandara", "Senanayake",
                                "Rathnayake", "Dissanayake", "Wijerathne", "Samaranayake", "Jayasekara", "Ranasinghe",
                                "Weerasinghe" };
                ...
                Tag mlTag = tagRepository.findByName("machine-learning").orElseGet(() -> tagRepository
                                .save(Tag.builder().name("machine-learning").category("Computer Science").build()));
                Tag medTag = tagRepository.findByName("biology").orElseGet(
                                () -> tagRepository.save(Tag.builder().name("biology").category("Medicine").build()));
                Tag mathTag = tagRepository.findByName("mathematics").orElseGet(() -> tagRepository
                                .save(Tag.builder().name("mathematics").category("Mathematics").build()));
                ...
                for (int i = 0; i < 20; i++) {
                        ...
                        if (groupIndex == 0) {
                                ...
                                myQuestionBody = "I've been studying this topic but I'm struggling to fully grasp the concepts. "
                                                + csQuestions[i % csQuestions.length];
                                peerAnswerBody = csAnswers[i % csAnswers.length];
                                msgContent = "Hey everyone! Glad to join the " + myGroup.getName()
                                                + " group. Looking forward to learning together!";
                        } else if (groupIndex == 1) {
                                myGroup = medGroup;
                                myTag = medTag;
                                myQuestionTitle = medQuestions[i % medQuestions.length];
                                myQuestionBody = "Could someone clarify this for me? "
                                                + medQuestions[i % medQuestions.length];
                                peerAnswerBody = medAnswers[i % medAnswers.length];
                                msgContent = "Hi guys! I'm really interested in " + myGroup.getName()
                                                + " and hope to share some great resources.";
                        } else {
                                myGroup = mathGroup;
                                myTag = mathTag;
                                myQuestionTitle = mathQuestions[i % mathQuestions.length];
                                myQuestionBody = "I have an upcoming exam and need help with this: "
                        ...
                        // To simulate community, let the *previous* student in the same group answer
                        // it, if i >= 3
                        if (i >= 3) {
                ...
                System.out.println(
                                "Seeded 20 students with group-specific interconnected data (June-August timeline).");
        }
