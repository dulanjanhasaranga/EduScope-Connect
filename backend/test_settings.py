...
req2 = urllib.request.Request("http://localhost:8080/api/admin/settings", headers={'Authorization': f'Bearer {token}'})
try:
    response2 = urllib.request.urlopen(req2)
    settings = json.loads(response2.read().decode('utf-8'))
    print("Settings count:", len(settings))
    print("Settings:", settings)
except urllib.error.HTTPError as e:
    print(f"Settings failed: {e.code} {e.read().decode('utf-8')}")
