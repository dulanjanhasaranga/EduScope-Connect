...
import { BarChart3, TrendingUp, Users, BrainCircuit, ArrowRight, Activity, BookOpen, Presentation, RefreshCcw } from 'lucide-react';
...
export default function FacultyLensSection() {
  ...
  const toggleSemester = () => {
    setSemester(prev => prev === 'This Semester' ? 'Next Semester' : 'This Semester');
  };
  ...
  const data = {
    'This Semester': {
      ...
      research: 35,
      service: 20,
      robert: { load: 110, color: 'red' },
      sarah: { load: 85, color: 'emerald' }
    },
    'Next Semester': {
      teaching: 55,
      research: 25,
      service: 20,
      robert: { load: 95, color: 'amber' },
      sarah: { load: 105, color: 'red' }
    }
  };

  const currentData = data[semester];
              ...
              <div className="p-6">
                <div className="flex items-center justify-between mb-8">
                  ...
                  <button 
                    ...
                    className="flex items-center gap-1.5 text-xs font-semibold text-blue-600 bg-blue-50 hover:bg-blue-100 px-3 py-1.5 rounded-lg transition-colors cursor-pointer"
                  >
                ...
                <div className="grid grid-cols-3 gap-4 mb-8">
                  <div 
                    onMouseEnter={() => setIsHovering('teaching')}
                    onMouseLeave={() => setIsHovering(null)}
                    className="p-4 rounded-xl border border-slate-100 bg-slate-50 shadow-sm hover:shadow-md transition-all cursor-default relative overflow-hidden"
                  >
                    <AnimatePresence>
                      {isHovering === 'teaching' && (
                        <motion.div 
                          initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}
                          className="absolute inset-0 bg-indigo-500 flex items-center justify-center text-white text-xs font-bold text-center p-2 rounded-xl"
                        >
                          +5% vs <br/> last year
                        </motion.div>
                      )}
                    ...
                    <motion.div layout className="text-2xl font-bold text-slate-900">{currentData.teaching}%</motion.div>
                    <div className="text-xs text-slate-500 font-medium">Teaching</div>
                  </div>
                  <div 
                    onMouseEnter={() => setIsHovering('research')}
                    onMouseLeave={() => setIsHovering(null)}
                    className="p-4 rounded-xl border border-slate-100 bg-slate-50 shadow-sm hover:shadow-md transition-all cursor-default relative overflow-hidden"
                  >
                    <AnimatePresence>
                      {isHovering === 'research' && (
                        <motion.div 
                          initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}
                          className="absolute inset-0 bg-emerald-500 flex items-center justify-center text-white text-xs font-bold text-center p-2 rounded-xl"
                        >
                          Target: <br/> 40%
                        </motion.div>
                      )}
                    ...
                    <motion.div layout className="text-2xl font-bold text-slate-900">{currentData.research}%</motion.div>
                  ...
                  >
                    <AnimatePresence>
                      {isHovering === 'service' && (
                        <motion.div 
                          initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}
                          className="absolute inset-0 bg-amber-500 flex items-center justify-center text-white text-xs font-bold text-center p-2 rounded-xl"
                        >
                          On track
                        </motion.div>
                      )}
                    ...
                    <motion.div layout className="text-2xl font-bold text-slate-900">{currentData.service}%</motion.div>
                  ...
                  <div className="flex items-center gap-4">
                    ...
                    <div className="flex-1">
                      <div className="flex justify-between mb-1.5">
                        ...
                        <motion.span 
                          layout
                          className={`text-xs font-bold text-${currentData.robert.color}-600 bg-${currentData.robert.color}-50 px-2 py-0.5 rounded-full`}
                        >
                          {currentData.robert.load}% Load
                        </motion.span>
                      </div>
                      <div className="w-full bg-slate-100 rounded-full h-2.5 overflow-hidden">
                        <motion.div 
                          initial={false}
                          animate={{ width: `${Math.min(currentData.robert.load, 100)}%` }}
                          transition={{ duration: 0.5, ease: "easeInOut" }}
                          className={`bg-gradient-to-r from-${currentData.robert.color}-400 to-${currentData.robert.color}-500 h-2.5 rounded-full`} 
                        ></motion.div>
                      </div>
                    </div>
                  </div>
                  <div className="flex items-center gap-4">
                    ...
                    <div className="flex-1">
                      <div className="flex justify-between mb-1.5">
                        ...
                        <motion.span 
                          layout
                          className={`text-xs font-bold text-${currentData.sarah.color}-600 bg-${currentData.sarah.color}-50 px-2 py-0.5 rounded-full`}
                        >
                          {currentData.sarah.load}% Load
                        </motion.span>
                      </div>
                      <div className="w-full bg-slate-100 rounded-full h-2.5 overflow-hidden">
                        <motion.div 
                          initial={false}
                          animate={{ width: `${Math.min(currentData.sarah.load, 100)}%` }}
                          transition={{ duration: 0.5, ease: "easeInOut" }}
                          className={`bg-gradient-to-r from-${currentData.sarah.color}-400 to-${currentData.sarah.color}-500 h-2.5 rounded-full`} 
                        ></motion.div>
                      </div>
                    </div>
                  </div>
